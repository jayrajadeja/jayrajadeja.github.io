...
- Domain: backend & distributed systems, full-stack. Languages: Go, TypeScript. Strong database work — MySQL query/index optimization, null-safe equality (`<=>`).

## Real work (interview-defensible — `/work`)
- Full-stack work on SAFE's enterprise cyber-risk-quantification platform (Go backend + React/TypeScript frontend).
